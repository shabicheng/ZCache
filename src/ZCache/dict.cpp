...
dict *dictCreate(dictType *type,
        void *privDataPtr)
{
    dict *ht = (dict *)_dictAlloc(sizeof(*ht));
...
int dictExpand(dict *ht, unsigned int size)
{
    ...
    n.table = (dictEntry **)_dictAlloc(realsize*sizeof(dictEntry*));
...
int dictAdd(dict *ht, void *key, void *val)
{
    ...
    entry = (dictEntry *)_dictAlloc(sizeof(*entry));
...
int dictReplace(dict *ht, void *key, void *val)
{
    ...
    * the element already exists. */
    if ((index = _dictKeyIndex(ht, key)) == -1)
    {
        ...
        dictSetHashVal(ht, entry, val);
        // return error means the key exist
        return DICT_ERR;
    }
...
{
    dictIterator *iter = (dictIterator *)_dictAlloc(sizeof(*iter));
...
static void *_dictStringCopyHTKeyDup(void *privdata, const void *key)
{
    ...
    char *copy = (char *)_dictAlloc(len+1);
